...
Client.prototype.queryCollection = function (collection, options, callback) {
    var self = this
    , queryOptions = { 'query': utils.buildRavenQuery(options.query), pageSize: options.pageSize };
        ...
        if (!error && result.statusCode === 200 && options.waitForNonStaleResults) {
            ...
            if (data.IsStale) {
                return setTimeout(function () {
                    self.queryCollection(collection, queryOptions, callback);
                }, 500);
            } else {
                return callback && callback(error, { result: data, response: result });
            }
        }
        callback(error, { result: result.statusCode === 200 ? result.asJson() : null, response: result });
    });
};
