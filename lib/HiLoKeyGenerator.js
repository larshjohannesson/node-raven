...
HiLoKeyGenerator.prototype._getNextMax = function (entityName, callback) {
    ...
    self.client.getDocument(self.documentPrefix + entityName, function (error, result) {
        ...
        if (doc) {
            ...
            if (doc.max) {
                // We have the older format max document, convert to the new Max that is compatible to the Raven.Client library used in the Magnum.Web project
                doc.Max = doc.max;
                delete(doc.max);
            }
            ...
            doc = { id: self.documentPrefix + entityName, Max: self.capacity, '@metadata': { 'etag': '00000000-0000-0000-000000000000'} };
            self.client.putDocument(self.documentPrefix + entityName, doc, function (error, result) {
                if (error || !result) { return callback && callback(error); }
