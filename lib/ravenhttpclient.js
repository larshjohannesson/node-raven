...
RavenHttpClient.prototype.wrapRequest = function (req, callback) {
  ...
  if (req.json) {
    ...
    Date.prototype.toJSON = function (key) {
        ...
        + pad(Math.floor(this.getUTCSeconds())) + '.0000000+00:00' // VERY strange. RavenDB seems to demand to have exactly 7 digits in decimal part for a valid date..
    };
...
//Perform a DELETE request against the server
// ========================================
...
RavenHttpClient.prototype.remove = function (path, headers, callback) {
    var self = this;

    self.wrapRequest({
        method: 'DELETE',
        uri: self.buildUrl(path, null),
        json: {},
        headers: headers
    }, callback);
};
